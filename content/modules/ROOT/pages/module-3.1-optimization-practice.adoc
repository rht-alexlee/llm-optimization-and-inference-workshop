...
. First we'll need to re-deploy the granite-3.3-8b-instruct. Open a terminal and login to your OpenShift cluster. Recreate our project name `rhaiis-demo`.
+
...
[source,sh,role=execute]
----
helm install granite-8b-instruct workshop_code/deploy_vllm/vllm-kserve -f workshop_code/deploy_vllm/vllm_rhoai_custom_2/values.yaml 
----
